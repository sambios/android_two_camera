...
void enumSupportedFromats(int i)
{
    ...
    LOGI("Video Capture enumerats supported image formats:");
    while (ioctl(fd[i], VIDIOC_ENUM_FMT, &fmt) == 0) {
		...
		if (-1 == xioctl (fd[i], VIDIOC_S_CROP, &crop)) {
			switch (errno) {
				case EINVAL:
					break;
				default:
                    //LOGE("xioctl VIDIOC_S_CROP err=%s\n", strerror(errno));
					break;
			}
		}
	} else {
	...
	for(j=0 ; j<CAMERANUM ; j++){
		...
		for(int line=0 ; line < height ; line++) {
            for(int col = 0; col < width; ++col){
			  colors[line* width + col] =  lrgb[line * info.width + col];
            }
		}
